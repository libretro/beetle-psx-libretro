...
`formats/chd`, `tools/flac` for `formats/flac`, `tools/mpeg1` for
`formats/mpeg1`, `tools/encodings` for `encodings`, `tools/formats` for
the rest of `formats`.

RetroArch carries `tools/cdfs` and `tools/cheevos` as well; they are
omitted here because this core has neither `formats/cdfs` nor
`deps/rcheevos` to test.
