...
{
 if(strcasecmp(fp->f_ext, "ws") && strcasecmp(fp->f_ext, "wsc") && strcasecmp(fp->f_ext, "wsr"))
  return(FALSE);
...
static int Load(const char *name, MDFNFILE *fp)
{
 ...
 real_rom_size = (fp->f_size + 0xFFFF) & ~0xFFFF;
 ...
 memcpy(wsCartROM + (rom_size - real_rom_size), fp->f_data, fp->f_size);
 ...
 WSwan_MemoryInit(MDFN_GetSettingB("wswan.language"), wsc, SRAMSize, /* IsWSR */false); // EEPROM and SRAM are loaded in this func.
 WSwan_GfxInit();
