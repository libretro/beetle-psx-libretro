...
static void DecodeXAADPCM(const uint8 *input, int16 *output, const unsigned shift, const unsigned weight)
{
   ...
   for(i = 0; i < 28; i++)
   {
      int32 sample = ((int16)(input[i] << 8)) >> shift;
...
/* Remove this function when we have better seek emulation; it's here because the 
 * Rockman Complete Works games(at least 2 and 4) apparently have finicky fubared CD
 * access code. */
...
/*
 Play command with a track argument that's not a valid BCD quantity causes interesting 
 half-buggy behavior on an actual PS1(unlike some of the other commands,
